...
para_desc="入口参数2个: 1.cube_name; 2.startDate"
if [[ "$2" = "" ]]; then
...
fn_parse_input_date "$startDate"
startTimeStamp=`date -d "$DAY_ID 00:00:00" +%s`  
startTimeStampMs=$(($startTimeStamp*1000)) #将current转换为时间戳，精确到毫秒  
endTimeStampMs=$(($startTimeStampMs + $onedayTime)) #当前时间偏移24小时
